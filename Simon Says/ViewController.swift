...
//  Simon Says
//
//  Created by Marcus Dantas on 19/04/2021.
//
...
class ViewController: UIViewController {
    ...
    func playSequence() {
        if sequenceIndex < colorSequence.count {
            ...
            actionButton.setTitle("Tap the right colors!", for: .normal)
            for button in colorButtons {
    ...
    func flash(button: CircularButton) {
        UIView.animateKeyframes(withDuration: 0.5, delay: 0.2, animations: {
            button.alpha = 1.0
            button.alpha = 0.5
        }) { (bool) in
            self.playSequence()
    ...
    @IBAction func colorButtonHandler(_ sender: CircularButton) {
        ...
        if colorsToTap.isEmpty {
            ...
            updateScoreLabels()
            switchPlayers()
            actionButton.setTitle("Tap to continue!", for: .normal)
            actionButton.isEnabled = true
        }
    }
    ...
    @IBAction func actionButtonHandler(_ sender: UIButton) {
        sequenceIndex = 0
        actionButton.setTitle("Memorize the sequence", for: .normal)
